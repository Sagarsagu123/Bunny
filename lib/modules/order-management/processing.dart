...
class Processing extends StatefulWidget {
  ...
  @override
  State<Processing> createState() => ProcessingOrderState();
}

class ProcessingOrderState extends State<Processing> {
  ...
  Future processingOrder() async {
    var jsonData = await orderController.processingOrder();
    if (jsonData != null && jsonData['code'] == 400) {
      setState(() {
        orderController.processingOrderArray = [];
      });
    } else {
      setState(() {
        orderController.processingOrderArray =
            jsonData['data']["order_details"];
      });
      return jsonData;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: _scaffoldKey,
      body: orderController.processingOrderArray.isNotEmpty
          ? RefreshIndicator(
              color: Colors.white,
              edgeOffset: 10.0,
              displacement: 40.0,
                ...
                child: Column(
                  children: [
                    StaticGrid(
                      columnCount: 1,
                      children:
                          orderController.processingOrderArray.map((order) {
                    ...
                    Align(
                        ...
                        child: Text(
                          "${orderController.processingOrderArray.length}",
                          style: SafeGoogleFont("Poppins",
          ...
          ListTile(
            title: Row(
              children: [
                ...
                Text(
                    ' ${orderDetails["self_pickup"] == "Yes" ? "Self pickUp" : "Door Delivery"}'),
              ],
            ),
          ),
        ],
